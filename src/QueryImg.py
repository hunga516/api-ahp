...
# Khai báo Flask, chỉ định folder chứa template là thư mục hiện tại (src)
app = Flask(__name__, template_folder=".")

# Lấy chuỗi kết nối MongoDB từ biến môi trường (có thể đặt mặc định nếu cần)
MONGO_URI = os.environ.get("MONGODB_URI", "mongodb+srv://opolo4847:moips103@cluster0.ikaskks.mongodb.net/")
...
# Route tìm kiếm theo các tham số: nếu không có tham số nào thì trả về tất cả document
@app.route('/data', methods=['GET'])
def get_data():
    query = {}
    ...
    # Nếu query rỗng, sẽ trả về tất cả document
    docs_cursor = collection.find(query)
    results = []
    for doc in docs_cursor:
        doc["_id"] = str(doc["_id"])
        for key, value in doc.items():
            if isinstance(value, Decimal128):
                doc[key] = float(value.to_decimal())
        # Làm sạch trường imageURL (chỉ lấy tên file)
        if "imageURL" in doc:
            # Nếu imageURL chứa đường dẫn, lấy phần basename (ví dụ: "binhthanh.jpg")
            doc["imageURL"] = os.path.basename(doc["imageURL"])
        results.append(doc)
...
# Route phục vụ ảnh (ảnh nằm trong thư mục images)
@app.route('/images/<path:filename>')
def serve_image(filename):
    image_folder = os.path.join(os.path.dirname(__file__), 'images')
    return send_from_directory(image_folder, filename)
